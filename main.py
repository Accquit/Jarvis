...
_MEMORY_EVERY_N_TURNS = 1
_last_memory_input    = ""

# ── Wake Word State ───────────────────────────────────────────────────────────
ACTIVE_TIMEOUT   = 12        # seconds active after last turn_complete
_wake_state      = "sleeping" # "sleeping" | "active"
_wake_lock       = threading.Lock()
_wake_timer: threading.Timer | None = None

def _go_to_sleep(jarvis_instance):
    global _wake_state
    with _wake_lock:
        _wake_state = "sleeping"
    print("[JARVIS] 😴 Sleeping — waiting for wake word")
    jarvis_instance.ui.write_log("SYS: Sleeping. Say 'Hey Jarvis' to wake me.")
    jarvis_instance.ui.set_sleeping(True)

def _wake_up(jarvis_instance):
    global _wake_state
    with _wake_lock:
        _wake_state = "active"
    print("[JARVIS] 👂 Wake word detected!")
    jarvis_instance.ui.write_log("SYS: I'm listening...")
    jarvis_instance.ui.set_sleeping(False)
    _reset_wake_timer(jarvis_instance)

def _reset_wake_timer(jarvis_instance):
    global _wake_timer
    if _wake_timer and _wake_timer.is_alive():
        _wake_timer.cancel()
    _wake_timer = threading.Timer(
        ACTIVE_TIMEOUT, lambda: _go_to_sleep(jarvis_instance)
    )
...
def _update_memory_async(user_text: str, jarvis_text: str) -> None:
    ...
    try:
        ...
        # --- Deep Vector Memory Extract ---
        # First, extract any standalone facts for the semantic database.
        # This should run on NOVEL information, regardless of whether it fits the specific JSON schema.
        vector_extract = model.generate_content(
            f"Extract any standalone facts, user opinions, or historical context "
            f"from this interaction that would be useful for a personal assistant to "
            f"remember long-term (e.g. 'User likes the movie The Room', 'User is working on a python project').\n"
            f"Return ONE concise sentence, or EMPTY IF NOTHING NOVEL.\n\n"
            f"Message: {text[:500]}"
        ).text.strip()
        
        if vector_extract and "EMPTY" not in vector_extract.upper() and len(vector_extract) > 10:
             vector_memory.add_memory(vector_extract)
...
TOOL_DECLARATIONS = [
    {
        "name": "recall_memory",
        "description": (
            "Dips into your deep long-term memory to search for past facts, conversations, "
            "or context about the user. MUST use this tool if the user asks you "
            "'Do you remember...', 'What did I say earlier...', or asks about a past event/preference "
            "that is NOT in your immediate context."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {
                    "type": "STRING",
                    "description": "Keywords to search your memory database for (e.g. 'favorite movie', 'project name')"
                }
            },
            "required": ["query"]
        }
    ...
    {
        "name": "schedule_proactive_alert",
        "description": (
            "Schedules an autonomous spoken alert or reminder from you in the specific future. "
            "Use this when the user says: 'Remind me in X minutes to...', 'Check on me in X minutes', "
            "'Interrupt me in X minutes', etc. You will autonomously speak the instruction when the delay completes."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "delay_minutes": {
                    "type": "NUMBER",
                    "description": "How many minutes to wait before speaking (e.g. 1.5, 5, 10, 60)"
                },
                "instruction": {
                    "type": "STRING",
                    "description": "The EXACT text you should speak out loud when the timer finishes. (e.g. 'Sir, it has been 10 minutes, the pizza is ready.')"
                }
            },
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        self.out_queue      = None
        self._loop          = None
        self.proactive      = ProactiveEngine(speak_callback=self.speak)
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "schedule_proactive_alert":
                delay_mins = float(args.get("delay_minutes", 1))
                instruction = args.get("instruction", "Sir, I have a reminder for you.")
                r = self.proactive.schedule(delay_minutes=delay_mins, instruction=instruction)
                result = r

            elif name == "recall_memory":
                db_results = await loop.run_in_executor(
                    None, lambda: vector_memory.search_memory(args.get("query", ""))
                )
                if db_results:
                    result = f"I remembered these facts from past interactions: {', '.join(db_results)}"
                else:
                    result = "I couldn't find any relevant memories matching that query."
    ...
    async def _listen_audio(self):
        ...
        # ── load openwakeword (optional) ──────────────────────────────────
        oww = None
        try:
            from openwakeword.model import Model as OWWModel
            oww = OWWModel(wakeword_models=["hey_jarvis"],
                           inference_framework="onnx")
            print("[JARVIS] 🔍 Wake word model loaded — say 'Hey Jarvis'")
            self.ui.set_sleeping(True)
            self.ui.write_log("SYS: Say 'Hey Jarvis' to activate.")
        except Exception as e:
            print(f"[JARVIS] ⚠️  openwakeword unavailable ({e}) — always-on mode")
        ...
        try:
            while True:
                ...
                if state == "sleeping":
                    # feed raw PCM to wake word detector
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            # after Jarvis finishes a turn, reset the sleep timer
                            # so the user has time to ask a follow-up
                            with _wake_lock:
                                if _wake_state == "active":
                                    _reset_wake_timer(self)
